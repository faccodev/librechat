...
import { validateWorkspaceSubdir, getSafePaths, selectExecutor, runCode, runFile } from "./runner.js";
import {
...
describe("getSafePaths with explicitWorkspacePath (project context)", () => {
  ...
  it("accepts a path under the second root when WORKSPACE_ROOTS has multiple", async () => {
    ...
    try {
      ...
      // slash-normalised form — the runner converts backslashes for
  ...
  it("allows a path that does not exist yet (project pinned to a future dir)", async () => {
    ...
    //
    // Set up WORKSPACE_ROOTS explicitly here — the previous tests in
...
describe("parseProjectContextHeader (AD-3 unified contract)", () => {
  ...
  it("throws InvalidProjectContextError on non-base64 input", () => {
    expect(() => parseProjectContextHeader("not-base64!!!")).toThrow(
      InvalidProjectContextError,
    );
  });
  ...
  it("throws on valid base64 but invalid JSON", () => {
    ...
    expect(() => parseProjectContextHeader(bad)).toThrow(
      InvalidProjectContextError,
    );
  });
  ...
  it("throws when projectId is missing / non-string / empty", () => {
    ...
    const emptyId = Buffer.from(JSON.stringify({ projectId: "", workspacePath: "/x" }), "utf8").toString("base64");
    const numId = Buffer.from(JSON.stringify({ projectId: 42, workspacePath: "/x" }), "utf8").toString("base64");
  ...
  it("throws when workspacePath is missing / non-string / empty", () => {
    ...
    const emptyPath = Buffer.from(JSON.stringify({ projectId: "a", workspacePath: "" }), "utf8").toString("base64");
...
describe("parseWorkspaceRoots", () => {
  ...
  it("splits multiple roots on comma and trims whitespace", () => {
    // `path.resolve` is platform-aware — on Linux it returns the
...
describe("getWorkspaceRoots (with env cache)", () => {
  ...
  it("caches the parsed roots on first call", () => {
    ...
    // Mutate env + read again — should still be the cached value.
...
describe("validateWorkspacePathAgainstRoots (realpath + strict-subdir)", () => {
  ...
  it("rejects a path that escapes the root", async () => {
    await expect(
      validateWorkspacePathAgainstRoots("/etc/passwd", [tmpRoot]),
    ).rejects.toThrow(/Path escapes workspace sandbox/);
  });

  it("rejects the root itself (must be a strict subdir)", async () => {
    await expect(
      validateWorkspacePathAgainstRoots(tmpRoot, [tmpRoot]),
    ).rejects.toThrow(/Path escapes workspace sandbox/);
  });

  it("allows a non-existent path (project pinned to a future dir)", async () => {
    const canonical = await validateWorkspacePathAgainstRoots(
      path.join(tmpRoot, "not-yet"),
      [tmpRoot],
    );
  ...
  it("blocks a symlink that resolves outside the root", async () => {
    ...
    // or developer mode to be enabled — without either, `fsp.symlink`
    ...
    try {
      ...
      await expect(
        validateWorkspacePathAgainstRoots(linkPath, [tmpRoot]),
      ).rejects.toThrow(/Path escapes workspace sandbox/);
    } finally {
...
describe("runCode / runFile with projectContext", () => {
  ...
  beforeEach(() => {
    ...
    process.env.WORKSPACE_ROOTS = tmpRoot;
    resetWorkspaceRootsCache();
    execFileSpy = jest.spyOn(require("child_process"), "execFile");
    // Minimal mock: return a successful empty result.
    (execFileSpy as unknown as jest.Mock).mockImplementation(
      (
        _cmd: unknown,
        _args: unknown,
        _opts: unknown,
  ...
  afterEach(() => {
    process.env = { ...ORIGINAL_ENV };
    resetWorkspaceRootsCache();
    (execFileSpy as unknown as jest.Mock).mockRestore();
  });
  ...
  it("runCode adds --label project=<id> when projectContext is provided", async () => {
    ...
    await runCode(
      ...
      "",
      5,
      { projectId: "abc-123", workspacePath: projectDir },
    );
    expect(execFileSpy).toHaveBeenCalledTimes(1);
    const args = (execFileSpy.mock.calls[0] as unknown[])[1] as string[];
  ...
  it("runFile adds --label project=<id> when projectContext is provided", async () => {
    ...
    await runFile(
      ...
      "sh",
      5,
      { projectId: "p2", workspacePath: projectDir },
    );
    expect(execFileSpy).toHaveBeenCalledTimes(1);
    const args = (execFileSpy.mock.calls[0] as unknown[])[1] as string[];
    const labelIdx = args.indexOf("--label");
  ...
  it("strips shell-metacharacters from the projectId before emitting the label", async () => {
    ...
    await runCode(
      ...
      "",
      5,
      { projectId: "evil;rm -rf /", workspacePath: projectDir },
    );
    const args = (execFileSpy.mock.calls[0] as unknown[])[1] as string[];
    const labelIdx = args.indexOf("--label");
...
describe("AsyncLocalStorage context (request -> tool handler)", () => {
  ...
  it("getCurrentProjectContext reads the value set inside the store", () => {
    requestContextStore.run(
      { projectContext: { projectId: "x", workspacePath: "/x" } },
      () => {
  ...
  it("the value does not leak across concurrent requests", async () => {
    // Two parallel ALS runs must each see their own context — this
