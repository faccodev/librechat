...
/* ─── Nav items ─────────────────────────────────────────────────────────── */
type NavSection = 'users' | 'whitelabel';
  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors ${
        isSelected
          ? 'bg-surface-tertiary border border-border-heavy'
    ...
    >
      <div className="relative flex-shrink-0">
        {user.avatar ? (
          ...
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-surface-tertiary text-sm font-semibold text-text-primary border border-border-light">
            {initial}
          </div>
        )}
        {isAdmin && (
...
interface UserDetailPanelProps {
  user: {
    ...
    avatar?: string; role: string; provider: string;
    createdAt?: string; updatedAt?: string; workspaceSubdir?: string | null;
  };
  onClose: () => void;
  onUserUpdate: () => void;
}
...
function UserDetailPanel({ user, onClose, onUserUpdate }: UserDetailPanelProps) {
  ...
  const handleRoleChange = (newRole: string) => {
    ...
    updateRoleMutation.mutate(newRole, {
      ...
      onError: (err: any) => { setRole(user.role); alert(err.message || 'Failed to update user role'); },
    });
  };
    ...
    <div className="flex h-full flex-col gap-4">
      <div className="flex items-center gap-3">
        <button type="button" onClick={onClose}
          className="rounded-lg p-1 text-text-secondary hover:bg-surface-secondary" aria-label="Back">
      ...
      <div className="rounded-xl border border-border-light bg-surface-secondary p-4">
        <div className="flex items-center gap-3">
          {user.avatar
            ...
            : <div className="flex h-12 w-12 items-center justify-center rounded-full bg-surface-tertiary text-lg font-bold text-text-primary border border-border-light">{initial}</div>
          }
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={label} className="flex items-center gap-2 rounded-lg border border-border-light bg-surface-primary px-3 py-2">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-border-light bg-surface-primary px-3 py-2">
          ...
          <div className="flex-1">
            ...
            <select value={role} onChange={(e) => handleRoleChange(e.target.value)}
              ...
              className="mt-0.5 block w-full rounded border-0 bg-transparent p-0 text-sm font-medium text-text-primary focus:ring-0 focus:outline-none cursor-pointer">
      ...
      <div className="mt-auto border-t border-border-light pt-4 space-y-2">
        {isLocal && !isSelf && (
          <button type="button" onClick={() => setResetPasswordOpen(true)}
        ...
        {!isSelf ? (
          confirmDelete ? (
            <div className="space-y-2 rounded-lg border border-red-300 bg-red-50 p-3 dark:border-red-800 dark:bg-red-950/30">
              <p className="text-xs font-medium text-red-700 dark:text-red-300">
                Excluir <strong>{user.name || user.email}</strong> permanentemente? Esta ação não pode ser desfeita.
              </p>
              <div className="flex justify-end gap-2">
                <button type="button" onClick={() => setConfirmDelete(false)} disabled={deleteUserMutation.isLoading}
                ...
                <button type="button" disabled={deleteUserMutation.isLoading}
                  onClick={() => deleteUserMutation.mutate(user.id, {
                    onSuccess: () => { setConfirmDelete(false); onUserUpdate(); onClose(); },
          ...
          <p className="text-[10px] italic text-text-secondary">Você não pode excluir sua própria conta pelo painel admin.</p>
        )}
      </div>

      <AdminResetPasswordDialog open={isResetPasswordOpen} onOpenChange={setResetPasswordOpen}
        userId={user.id} userName={user.name || user.email || user.username || user.id} />
    </div>
  );
}

/* ─── CreateUserPanel ────────────────────────────────────────────────────── */
interface CreateUserPanelProps { onClose: () => void; onSuccess: (user: any) => void; }
...
function CreateUserPanel({ onClose, onSuccess }: CreateUserPanelProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    createUserMutation.mutate(
      { name, username: username || undefined, email, password, role, workspaceSubdir: workspaceSubdir.trim() || null },
      { onSuccess: (newUser) => onSuccess(newUser), onError: (err: any) => setError(err.message || 'Failed to create user') },
    );
  };

  const inputClass = 'h-9 w-full rounded-lg border border-border-light bg-surface-secondary px-3 text-sm text-text-primary focus:outline-none focus:ring-1 focus:ring-border-heavy';

  return (
    <form onSubmit={handleSubmit} className="flex h-full flex-col gap-4">
      <div className="flex items-center gap-3">
        <button type="button" onClick={onClose}
          className="rounded-lg p-1 text-text-secondary hover:bg-surface-secondary" aria-label="Cancel">
          <ChevronLeft className="size-4" />
        </button>
      ...
      <div className="space-y-3 flex-1 overflow-y-auto pr-1">
        {error && (
          <div className="rounded-lg bg-red-100 p-3 text-xs font-medium text-red-700 dark:bg-red-900/30 dark:text-red-400">{error}</div>
        )}
        {[
          { label: 'Full Name *', type: 'text', value: name, setter: setName, placeholder: 'John Doe', required: true },
          { label: 'Username', type: 'text', value: username, setter: setUsername, placeholder: 'johndoe', required: false },
          { label: 'Email Address *', type: 'email', value: email, setter: setEmail, placeholder: 'john@example.com', required: true },
          { label: 'Password *', type: 'password', value: password, setter: setPassword, placeholder: '••••••••', required: true },
          ...
          <div key={label} className="space-y-1">
            ...
            <input type={type} required={required} value={value}
              onChange={(e) => setter(e.target.value)} placeholder={placeholder} className={inputClass} />
          </div>
        ))}
        ...
        <div className="space-y-1">
          ...
          <div className="flex gap-2">
            <input type="text" value={workspaceSubdir} onChange={(e) => setWorkspaceSubdir(e.target.value)}
              placeholder="e.g. john" className={inputClass} />
            <button type="button" onClick={() => setIsWorkspacePickerOpen(true)}
          ...
          <AdminWorkspaceDialog open={isWorkspacePickerOpen} onOpenChange={setIsWorkspacePickerOpen}
            initialSubdir={workspaceSubdir} onSelect={(val) => setWorkspaceSubdir(val ?? '')} />
        </div>
      </div>
...
function UsersSection() {
  ...
  const usersQuery = useAdminUsers({ limit: PAGE_SIZE, offset: page * PAGE_SIZE }, { enabled: !isSearching });
  ...
  const displayedUsers = useMemo(() => {
    if (isSearching) {
      return (searchQuery$.data?.users ?? []).map((u) => ({
        ...
        avatar: u.avatarUrl, role: 'USER', provider: 'local', workspaceSubdir: null,
      }));
    }
  ...
  const selectedUserData = useMemo(() => displayedUsers.find((u) => u.id === selectedUser) ?? null, [displayedUsers, selectedUser]);
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      ...
      <div className="flex w-60 flex-shrink-0 flex-col border-r border-border-light">
        ...
        <div className="flex flex-shrink-0 items-center gap-2 p-3">
          <div className="relative flex-1">
            ...
            <input type="text" value={searchQuery}
              onChange={(e) => { setSearchQuery(e.target.value); setPage(0); setSelectedUser(null); setIsCreating(false); }}
          ...
          <button type="button"
            onClick={() => { setSelectedUser(null); setIsCreating(true); }}
          ...
          {isLoading ? (
            ...
            <div className="space-y-0.5">
              {displayedUsers.map((u) => (
                <AdminUserRow key={u.id} user={u} isSelected={selectedUser === u.id}
                  onClick={() => { setIsCreating(false); setSelectedUser(u.id === selectedUser ? null : u.id); }} />
              ))}
            </div>
          )}
        </div>
        ...
        {!isSearching && (
          <div className="flex-shrink-0 border-t border-border-light px-3 py-2">
            <span className="text-[10px] text-text-secondary">{totalUsers} total user{totalUsers !== 1 ? 's' : ''}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {isCreating ? (
          <CreateUserPanel onClose={() => setIsCreating(false)}
            onSuccess={(newUser) => { setIsCreating(false); usersQuery.refetch(); setSelectedUser(newUser.id); }} />
        ) : selectedUserData ? (
          <UserDetailPanel user={selectedUserData} onClose={() => setSelectedUser(null)} onUserUpdate={() => usersQuery.refetch()} />
        ) : (
          <div className="flex h-full flex-col items-center justify-center gap-3 text-text-secondary">
            ...
            <div className="text-center">
              ...
              <p className="text-xs">Clique em um usuário da lista para ver e gerenciar seus detalhes</p>
            </div>
          </div>
        )}
      </div>
    ...
    <OGDialog open={open} onOpenChange={handleOpenChange}>
      <OGDialogContent className="flex h-[90vh] max-h-[720px] w-[95vw] max-w-3xl flex-col gap-0 overflow-hidden border-border-light bg-surface-primary p-0 text-text-primary">
        ...
        <div className="flex flex-shrink-0 items-center gap-3 border-b border-border-light px-5 py-4">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-tertiary border border-border-light">
            <Shield className="size-4 text-text-primary" />
          </div>
          <div>
            <OGDialogTitle className="text-base font-semibold text-text-primary">Admin Panel</OGDialogTitle>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* ── Left nav sidebar ── */}
          <nav className="flex w-40 flex-shrink-0 flex-col border-r border-border-light bg-surface-secondary py-3 px-2 gap-0.5">
            {navItems.map(({ id, label, icon: Icon }) => (
              <button
                ...
                className={`flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm transition-colors ${
                  activeSection === id
                    ? 'bg-surface-primary font-semibold text-text-primary border border-border-light shadow-sm'
