...
describe('isWorkspaceAwareServer', () => {
  it.each(['filesystem', 'code-runner', 'transcribe'])(
    'recognizes "%s" as workspace-aware',
    (name) => {
  ...
  it('exposes the configured set', () => {
    expect(WORKSPACE_AWARE_MCP_SERVERS.has('filesystem')).toBe(true);
    expect(WORKSPACE_AWARE_MCP_SERVERS.has('code-runner')).toBe(true);
...
describe('buildWorkspaceHint', () => {
  it('includes the resolved sandbox root when provided', () => {
    ...
    expect(hint).toContain('MCP server: filesystem');
  });

  it('falls back to /workspaces/<subdir> when no resolved path is given', () => {
    const hint = buildWorkspaceHint({ workspaceSubdir: 'bob', serverName: 'code-runner' });
    expect(hint).toContain('Sandbox root: /workspaces/bob');
    expect(hint).toContain('MCP server: code-runner');
  });
...
describe('patchToolDescription', () => {
  it('appends the hint to a tool with no existing context marker', () => {
    ...
    patchToolDescription(tool, { user: { workspaceSubdir: 'alice' }, workspacePath: '/workspaces/alice' });
...
describe('patchToolDefinitions', () => {
  it('patches every tool whose server is workspace-aware', () => {
    const tools = {
      'filesystem__read_file': { description: 'Read a file.', schema: {} },
      'filesystem__list_dir': { description: 'List directory.', schema: {} },
      'code-runner__run_code': { description: 'Run code.', schema: {} },
    };
    ...
    expect(tools['filesystem__read_file'].description).toContain('[Workspace context]');
    expect(tools['filesystem__list_dir'].description).toContain('[Workspace context]');
    expect(tools['code-runner__run_code'].description).toContain('[Workspace context]');
  });
